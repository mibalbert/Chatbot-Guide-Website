 ...
 */
function createCardsCarousel2(cardsData) {
    let cards = "";
    cardsData.map((card_item) => {
        const item = `<div class="carousel_cards in-left">
    <a href=${card_item.href}><img class="cardBackgroundImage2" src=${card_item.image}></a>
    <div class="cardFooter2"> <span class="cardTitle2" title="${card_item.title}">${card_item.title}</span>
    <div class="cardDescription2"></div></div></div>`;
        cards += item;
    });
 ...
 */
function showCardsCarousel2(cardsToAdd) {
    const cards = createCardsCarousel2(cardsToAdd);
